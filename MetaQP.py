...
class MCTSnet:
    def __init__(self,
        ...
        if self.ver is not "best":
            ...
            self.best_mcts = MCTSnet(self.actions,
    ...
    def load_model(self, ver):
        try:
            return torch.load('checkpoints/models/mcts_%s.t7' % ver)
        except:
            print('Initialize new Network Weights for mcts_%s' % ver)
            return MCTS()

    def self_play(self, root_state, memories, best_player=1):
        results = {
            "new": 0, "best": 0, "draw": 0
        }
        self.mcts.eval()
        self.best_mcts.mcts.eval()
        for _ in tqdm(range(config.EPISODES)):
            ...
            # it would be great to play games in parallel, i.e. send in a batch
            # of 128 states, and output a move for each
            while not game_over:
                curr_player = turn%2
                turn += 1
                if curr_player == best_player:
                    state, memory = self.best_mcts.do_move(
                        state, turn, legal_actions, curr_player)
                else:
                    state, memory = self.do_move(state, turn, legal_actions, curr_player)
    ...
    def save_best_model(self):
        ...
        torch.save(self.mcts, "checkpoints/models/mcts_best.t7")
    ...
    def meta_self_play(self, state):
        self.meta_net.eval()
        memory = [dict() for _ in range(config.BATCH_SIZE)]
        orig_player = state[0][2][0]
        ...
        for i in range(config.BATCH_SIZE):
            random_policies[i] = np.random.dirichlet([config.ALPHA] * len(self.actions))
            ...
            #idea: scale random policies to legal moves to make them more valid
            #
            #going to assume state is a torch tensor
            states[i] = np.copy(state)

        random_policies_var = self.wrap_to_variable(random_policies, volatile=True)
        states_var = self.wrap_to_variable(states, volatile=True)

        Qs, weighted_random_policies, weighted_inferred_policies, optimal_policies = self.meta_net(states_var, 
        random_policies_var, weighted=True)

        improved_policy = weighted_random_policies + weighted_inferred_policies + optimal_policies
        improved_policy = self.correct_policy(improved_policy.data.numpy())
        memory[i]["improved_policy"] = np.copy(improved_policy)

        new_policies = [improved_policy*(1-config.EPSILON) + 
            random_policies[i]*config.EPISILON for i in range(config.BATCH_SIZE)]

        # new_states = torch.from_numpy(np.zeros(shape=config.BATCHED_SHAPE, dtype="float32"))
        # new_random_policies = torch.from_numpy(np.zeros(shape=config.BATCHED_SHAPE, dtype="float32"))
        
        skip_indices = dict()
        for i in range(config.BATCH_SIZE):
            skip_indices[str(i)] = False

        num_done = 0
        while True:
            if num_done == config.BATCH_SIZE:
                break
            for i, policy in enumerate(new_policies):
                if not skip_indices[i]:
                    idx = np.random.choice(self.actions, p=policy)
                    states[i][:2] = self.transition(state[i][:2], idx)
                    states[i][2] = (states[i][2][0]+1) % 2
                    legal_actions = self.get_legal_actions(states[i][:2])

                    if len(legal_actions) == 0:
                        memory[i]["result"] = 0
                        skip_indices[str(i)] = True
                        num_done += 1
                        continue

                    reward, game_over = self.calculate_reward(states[i][:2])

                    if game_over:
                        if states[i][2][0] == orig_player:
                            memory[i]["result"] = reward
                        else:
                            memory[i]["result"] = -1*reward
                        skip_indices[str(i)] = True
                        num_done += 1
                        continue
                    ...
                    random_policy_var = self.wrap_to_variable(random_policies[i], volatile=True).unsqueeze(0)
                    state_var = self.wrap_to_variable(states[i], volatile=True).unsqueeze(0)

                    Qs, weighted_random_policies, weighted_inferred_policies, optimal_policies = \
                        self.meta_net(random_policy_var, state_var, weighted=True)

                    new_policies[i] = weighted_random_policies[0] + weighted_inferred_policies[0] + \
                        optimal_policies[0]

                    new_policies[i] = self.correct_policy(new_policies[i].data.numpy())
    ...
    def train_memories(self, memories):
        ...
        if len(memories) < config.NUM_TASKS:
            print("Need {} tasks, have {}".format(config.NUM_TASKS, len(memories)))
            return
    ...
    def train_tasks(self, tasks):
        ...
        for task in tasks:
            states = np.zeros(shape=(config.N_way, config.CH, config.R, config.C))
            random_policies = np.zeros(shape=(config.N_way, config.R*config.C))
            results = np.zeros(shape=(config.N_way))
            improved_policies = np.zeros(shape=(config.N_way, config.R*config.C))         
            ...
            random_policies = self.wrap_to_variable(random_policies, volatile=True)
            results = self.wrap_to_variable(results, volatile=True)
            improved_policies = self.wrap_to_variable(improved_policies, volatile=True)

            Qs, _, inferred_policies, optimal_policies = \
                self.meta_net(states, random_policies, weighted=False)

            Q_loss += F.mse_loss(Qs, results)
            inferred_loss += torch.mm(inferred_policies.t(), torch.log(random_policies))
            optimal_loss += torch.mm(improved_policies[0].t(), torch.log(optimal_policies[0]))
    ...
    def do_move(self, state, turn, legal_actions, curr_player):
        ...
        inputs = torch.from_numpy(np.zeros(shape=config.BATCHED_SHAPE, dtype="float32"))
        for i in range(config.BATCH_SIZE):
    ...
    def train_minibatches(self, minibatches):
        for e in range(config.EPOCHS):
            ...
            for mb in minibatches:
                ...
                valids, values, policies, imagined_states, _ = self.mcts(mb_tensor, is_real_list)
                del mb_tensor
