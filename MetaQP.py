...
class MetaQP:
    ...
    def meta_self_play(self, state):
        ...
        random_policies_var = self.wrap_to_variable(
        ...
        improved_policy = weighted_random_policies + \
            weighted_inferred_policies + optimal_policies
        ...
        while True:
            ...
            for i, policy in enumerate(new_policies):
                if not skip_indices[i]:
                    ...
                    random_policies[i] = np.random.dirichlet(
                        [config.ALPHA] * len(self.actions))
                    random_policies[i] = self.correct_policy(
                        random_policies[i])

                    random_policy_var = self.wrap_to_variable(
                        random_policies[i], volatile=True).unsqueeze(0)
                    state_var = self.wrap_to_variable(
                        states[i], volatile=True).unsqueeze(0)

                    Qs, weighted_random_policies, weighted_inferred_policies, optimal_policies = \
                        self.meta_net(random_policy_var,
                                      state_var, weighted=True)

                    new_policies[i] = weighted_random_policies[0] + weighted_inferred_policies[0] + \
                        optimal_policies[0]

                    new_policies[i] = self.correct_policy(
                        new_policies[i].data.numpy())
